...
void opendata()
{
    ...
    if (zone>1) {
		if (zone>u1+1 && zone==u1+2 || zone==u1+u2+2 || zone==u1+2*u2+2)
			sprintf(filename,"%s/sol%d.log",dirname,u1+1);
		else
			sprintf(filename,"%s/sol%d.log",dirname,zone-1);
        FILE *f = fopen(filename,"r");
        i=1; j=1;
		if (zone<=u1 || zone!=u1+2 && zone!=u1+u2+2 && zone!=u1+2*u2+2) {
		...
		else if (zone==u1+u2+2) {
            while(fgets(buff, BUZZ_SIZE, f) != NULL) {
                if (buff[0]=='z' || buff[0]=='x') {
                    if (4 == sscanf(buff,"%*[^0123456789]%d%*[^0123456789]%d%*[^0123456789]%d%*[^0123456789]%f",&i1[i],&i2[i],&i3[i],&i4[i])) {
                        //cout << i1[i] << "\t" << i2[i] << "\t" << i3[i] << "\t" << i4[i] << "\n";
                        i++;
                    }
		...
		else if (zone==u1+2*u2+2) {
            while(fgets(buff, BUZZ_SIZE, f) != NULL) {
                if (buff[0]=='v' || buff[0]=='x') {
                    if (4 == sscanf(buff,"%*[^0123456789]%d%*[^0123456789]%d%*[^0123456789]%d%*[^0123456789]%f",&i1[i],&i2[i],&i3[i],&i4[i])) {
                        //cout << i1[i] << "\t" << i2[i] << "\t" << i3[i] << "\t" << i4[i] << "\n";
                        i++;
                    }
        ...
        if (nov[1]<P[1] || nov[2]<P[2] || nov[3]<P[3]) {
			if (zone>u1+1 && zone==u1+2 || zone==u1+u2+2 || zone==u1+2*u2+2)
				sprintf(filename,"%s/depot%d.txt",dirname,u1+1);
			else 
...
int main(int argc, char *argv[])
{
    ...
    if ((mfptr = fopen(filename,"w")) == NULL) {
        ...
        fprintf(mfptr,"F ");
        if (zone!=u1+u2+1 && zone!=u1+2*u2+1 && zone!=u1+3*u2+1)
            fprintf(mfptr,"- f1 - f2");
        fprintf(mfptr,"\n");
        ...
        fprintf(mfptr,"F ");
		for (i=m+1;i<=m+n;i++) {
			if (zone<=u1) fprintf(mfptr,"- %dy%d ",p[i],id[i]);
			else if (zone<=2*u1+1) fprintf(mfptr,"- %.1fy%d ",Pr[2]*p[i],id[i]);
			else if (zone<=3*u1+1) fprintf(mfptr,"- %.1fy%d ",Pr[3]*p[i],id[i]);
			else if (zone<=4*u1+1) fprintf(mfptr,"- %.1fy%d ",Pr[4]*p[i],id[i]);
		}
        fprintf(mfptr,"= 0");
        fprintf(mfptr,"\n");
        if (zone!=u1+u2+1 && zone!=u1+2*u2+1 && zone!=u1+3*u2+1) {
        ...
        if (zone<=u1) {
            ...
            for (j=1;j<=n+m;j++) {
                for (i=1;i<=n+m;i++) {
                    for (q=1;q<=nv;q++) {
                        if (fe[i][j][q]) {
                            fprintf(mfptr,"s%d - %.2fz%dz%dz%d ",id[j],ST,id[i],id[j],q);
                            fprintf(mfptr,">= 0\n");
                        }
                    }
                }
